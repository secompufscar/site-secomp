...
relacao_atividade_area = db.Table(
    "relacao_atividade_area",
...
relacao_atividade_participante = db.Table(
    "relacao_atividade_participante",
    Column("id", Integer, primary_key=True),
    Column("id_atividade", Integer, db.ForeignKey("atividade.id")),
...
relacao_atividade_patrocinador = db.Table(
    "relacao_atividade_patrocinador",
    Column("id", Integer, primary_key=True),
    Column("id_atividade", Integer, db.ForeignKey("atividade.id")),
    Column("id_patrocinador", Integer, db.ForeignKey("patrocinador.id")),
)
...
class Usuario(db.Model):
    ...
    permissoes_usuario = db.relationship(
        "Permissao", secondary=relacao_permissao_usuario, lazy=True, back_populates="usuarios"
    )
...
class Participante(db.Model):
    ...
    atividades = db.relationship(
        "Atividade", secondary=relacao_atividade_participante, lazy=True, back_populates="participantes"
    )
    ...
    def __repr__(self):
        return (
            self.usuario.primeiro_nome
            + " "
            + self.usuario.sobrenome
            + " <"
            + self.usuario.email
            + "><"
            + str(self.evento.edicao)
            + "ª edição>"
        )
...
class Ministrante(db.Model):
    ...
    atividades = db.relationship(
        "Atividade", secondary="relacao_atividade_ministrante", lazy=True, back_populates="ministrantes"
    )
    ...
    def __repr__(self):
        if (
            self is not None
            and self.usuario is not None
            and self.usuario.primeiro_nome is not None
            and self.usuario.sobrenome is not None
            and self.usuario.email is not None
        ):
...
class AreaAtividade(db.Model):
    ...
    atividades = db.relationship("Atividade", secondary=relacao_atividade_area, lazy=True, back_populates="areas")
...
class Atividade(db.Model):
    ...
    patrocinadores = db.relationship(
        "Patrocinador", secondary=relacao_atividade_patrocinador, lazy=True, back_populates="atividades"
    )

    areas = db.relationship("AreaAtividade", secondary=relacao_atividade_area, lazy=True, back_populates="atividades")
    ministrantes = db.relationship(
        "Ministrante", secondary="relacao_atividade_ministrante", lazy=True, back_populates="atividades"
    )
    participantes = db.relationship(
        "Participante", secondary=relacao_atividade_participante, lazy=True, back_populates="atividades"
    )
    ...
    def ministrantes_confirmados_atividade(self):
        ...
        for m in ministrantes:
            r = (
                ...
                .filter_by(id_ministrante=m.id, id_atividade=self.id, confirmado=True)
                .first()
            )
            if r is not None:
    ...
    def admin(self):
        r = (
            db.session.query(RelacaoAtividadeMinistrante)
            .filter_by(id_atividade=self.id, confirmado=True, admin_atividade=True)
            .first()
        )
...
class Evento(db.Model):
    ...
    patrocinadores = db.relationship(
        "Patrocinador", secondary=relacao_patrocinador_evento, lazy=True, back_populates="eventos"
    )
...
class Patrocinador(db.Model):
    ...
    eventos = db.relationship("Evento", secondary=relacao_patrocinador_evento, lazy=True, back_populates="patrocinadores")
    atividades = db.relationship(
        "Atividade", secondary=relacao_atividade_patrocinador, lazy=True, back_populates="patrocinadores"
    )
...
class Permissao(db.Model):
    ...
    id = Column(Integer, primary_key=True)
    nome = Column(String(100), nullable=False)
    usuarios = db.relationship("Usuario", secondary=relacao_permissao_usuario, lazy=True, back_populates="permissoes_usuario")
