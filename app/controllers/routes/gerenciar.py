...
gerenciar = Blueprint('gerenciar', __name__, static_folder='static',
...
def index():
    if possui_permissao(current_user) or current_user.is_admin():
        ...
        return render_template('gerenciar/gerenciar.html', usuario=current_user, permissoes=permissoes, form_login=form_login)
    else:
        abort(403)


@gerenciar.route('/estoque-camisetas')
@login_required
def estoque_camisetas():
    ...
    if "ALTERAR_CAMISETAS" in permissoes or current_user.is_admin():
        ...
        return render_template('gerenciar/controle_camisetas.html', camisetas=camisetas, usuario=current_user, participante=participante, form_login=form_login)
    else:
        abort(403)


@gerenciar.route('/estoque-camisetas/<tamanho>')
@login_required
...
@gerenciar.route('/cadastro-patrocinador', methods=['POST', 'GET'])
@login_required
def cadastro_patrocinador():
    ...
    if "CADASTRAR_PATROCINADOR" in permissoes or current_user.is_admin():
        ...
        if form.validate_on_submit():
            ...
            return render_template('gerenciar/cadastro_patrocinador.html', usuario=current_user, participante=participante, form=form, form_login=form_login)
    else:
        abort(403)


@gerenciar.route('/venda-kits', methods=['POST', 'GET'])
@login_required
def vender_kits():
    permissoes = current_user.getPermissoes()
    ...
    if "VENDA_PRESENCIAL" in permissoes or current_user.is_admin():
        ...
        if form.validate_on_submit() and form.participante.data is not None:
            ...
            if camiseta.quantidade_restante > 0:
                ...
                db.session.commit()
                return render_template('gerenciar/venda_de_kits.html', alerta=alerta,
                                       form=form, form_login=form_login, usuario=current_user, participante=meu_participante)
            elif camiseta.quantidade_restante == 0:
                return render_template('gerenciar/venda_de_kits.html', alerta="Sem estoque para " + camiseta.tamanho,
                                       form=form, form_login=form_login, usuario=current_user, participante=meu_participante)
        return render_template('gerenciar/venda_de_kits.html', alerta="Preencha o formulário abaixo",
                               form=form, form_login=form_login, usuario=current_user, participante=meu_participante)
    else:
        abort(403)

@gerenciar.route('/sorteio')
@login_required
def sorteia_usuario():
    permissoes = current_user.getPermissoes()
    if "SORTEAR" in permissoes or current_user.is_admin():
        ...
        return render_template('gerenciar/sortear_usuario.html', form=form, usuario=current_user, participante=participante, sorteando=False, form_login=form_login)
    else:
        abort(403)

@gerenciar.route('/sorteio/sortear', methods=["POST"])
@login_required
def sortear():
    permissoes = current_user.getPermissoes()
    if "SORTEAR" in permissoes or current_user.is_admin():
        ...
        if len(presencas) > 0:
            ...
            return render_template('gerenciar/sortear_usuario.html', usuario=current_user, form=form, participante=participante, sorteado=sorteado, sorteando=True, form_login=form_login)
        else:
            return render_template('gerenciar/sortear_usuario.html', usuario=current_user, form=form, vazio=True, sorteando=False, form_login=form_login)
    else:
        abort(403)

@gerenciar.route('/alterar-camisetas', methods=["GET", "POST"])
@login_required
    ...
    if "ALTERAR_CAMISETAS" in permissoes or current_user.is_admin():
        ...
        if form.validate_on_submit() and form.participante.data is not None:
            ...
            if camiseta.quantidade_restante > 0:
                ...
                db.session.add(participante)
                db.session.commit()
                return render_template('gerenciar/alterar_camisetas.html', participante=participante, camiseta=camiseta,
        ...
        return render_template('gerenciar/alterar_camisetas.html', form=form, form_login=form_login, usuario=current_user)
    else:
        abort(403)

@gerenciar.route('/listas', methods=["GET", "POST"])
@login_required
def listas():
    permissoes = current_user.getPermissoes()
    if "GERAR_LISTAS" in permissoes or current_user.is_admin():
        ...
        if(form.validate_on_submit()):
            if(form.tipo.data == 0):
                ...
                return render_template('gerenciar/listas_participante.html', atividade=atividade.titulo + ' - ' + atividade.tipo.nome,
            ...
            return render_template('gerenciar/listas_participante.html', form=form, form_login=form_login,
                                   usuario=current_user, participante=participante)
    else:
        abort(403)

@gerenciar.route("/email-custom", methods=["GET"])
@login_required
def email_custom():
    """
    ...
    if "ENVIAR_EMAIL" in permissoes or current_user.is_admin():
        ...
        return render_template('gerenciar/email_custom.html', form=form, form_login=form_login)
    else:
        abort(403)

@gerenciar.route('/gerar-url-conteudo', methods=['POST', 'GET'])
@login_required
        ...
        if form.validate_on_submit():
            ...
            if verifica_lista_emails(emails):
                ...
                if atividade is not None and tipo is not None:
                    ...
                    db.session.commit()
        return render_template("gerenciar/gerar_url_conteudo.html", form=form, dict_urls=get_urls_conteudo(request.url_root), form_login=form_login, url_root=request.url_root, usuario=current_user)
    else:
        abort(403)

@gerenciar.route('/crd-flag', methods=["GET", "POST"])
@login_required
def cadastro_flags():
    permissoes = current_user.getPermissoes()
    if "CONTEUDO" in permissoes or current_user.is_admin():
        ...
        if form.validate_on_submit():
            ...
            return render_template("gerenciar/crd_flags.html", form_login=form_login, form=form, cadastrado=True, desativada=False, flags=flags, usuario=current_user)
        else:
            flags = db.session.query(Flag).filter_by(ativa=True).all()
            return render_template("gerenciar/crd_flags.html", form_login=form_login, form=form, cadastrado=False, desativada=False, flags=flags, usuario=current_user)
    else:
        abort(403)

@gerenciar.route('/add-pontuacao', methods=["GET", "POST"])
@login_required
def add_pontuacao():
    permissoes = current_user.getPermissoes()
    if "CONTEUDO" in permissoes or current_user.is_admin():
        ...
        if form.validate_on_submit():
            ...
            db.session.flush()
            db.session.commit()
            return render_template("gerenciar/add_pont.html", ult_day=True, form_login=form_login, form=form, sucesso=True, usuario=current_user)
        else:
            return render_template("gerenciar/add_pont.html", ult_day=True, form_login=form_login, form=form, usuario=current_user)
    else:
        abort(403)


@gerenciar.route('/crd-flag/desativar/<id>', methods=["GET", "POST"])
@login_required
def desativar_flag(id):
    ...
    if "CONTEUDO" in permissoes or current_user.is_admin():
        ...
        db.session.commit()
        flags = db.session.query(Flag).filter_by(ativa=True).all()
        return render_template("gerenciar/crd_flags.html", form_login=form_login, form=form, cadastrado=False,
                               desativada=True, flags=flags, usuario=current_user)
    else:
        abort(403)

@gerenciar.route('/gerenciar-comprovantes', methods=['POST', 'GET'])
@login_required
        ...
        if form.validate_on_submit():
            if esta_preenchido(form.aprovar.data) and not esta_preenchido(form.desaprovar.data) and not esta_preenchido(form.rejeitar.data):
                ...
                if pagamento.rejeitado is not False and pagamento.metodo_pagamento == 'Comprovante':
                    ...
                    db.session.add(pagamento)
                    db.session.commit()
        return render_template('gerenciar/gerenciar_comprovantes.html', form=form, participante=participante, usuario=current_user, form_login=form_login, pagamentos=get_info_usuarios_envio_comprovante())
    else:
        abort(403)

@gerenciar.route('/cadastro-presencial-participante', methods=['POST', 'GET'])
@login_required
def cadastro_presencial_participante():
    if "CREDENCIAMENTO" in current_user.getPermissoes() or "ADMIN" in current_user.getPermissoes() or current_user.is_admin():
        ...
        if form.validate_on_submit():
            ...
            form.usuario.choices = get_usuarios_inscricao_pendente()
            return render_template('gerenciar/cadastro_participante_presencial.html', alerta=alerta,
                                   form=form, form_login=form_login, usuario=current_user, participante=meu_participante)
        return render_template('gerenciar/cadastro_participante_presencial.html', form=form, form_login=form_login,
