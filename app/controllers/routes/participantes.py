...
participantes = Blueprint('participantes', __name__, static_folder='static',
                         template_folder='templates', url_prefix='/participantes')
...
@participantes.route('/cadastro', methods=['POST', 'GET'])
def cadastro():
    """
...
def verificar_email():
    ...
    if email_confirmado():
        ...
        status = False
    return render_template('participantes/confirma_email.html', resultado=msg, status=status, ministrante=ministrante, form_login=form_login)
...
@participantes.route('/reenviar-email', methods=['POST', 'GET'])
@login_required
def reenviar_email():
...
@participantes.route('/cadastro-participante', methods=['POST', 'GET'])
@login_required
        ...
        if current_user.email_verificado:
            ...
            if participante is None:
                ...
                if form.validate_on_submit() and participante is None:
                    ...
                    return render_template('participantes/cadastro_participante.html', form=form, usuario=current_user)
            else:
...
@participantes.route('/alterar-dados-usuario', methods=['POST', 'GET'])
@login_required
def alterar_usuario():
    ...
    if form.validate_on_submit() and request.method == 'POST':
        ...
        return render_template('participantes/alterar_usuario.html', usuario=current_user, form=form, participante = db.session.query(Participante).filter_by(usuario=current_user).first())


@participantes.route('/dashboard', methods=['POST', 'GET'])
@login_required
...
def dashboard():
    if email_confirmado():
        ...
        return render_template('participantes/dashboard_usuario.html', title='Dashboard', usuario=current_user, participante=participante)
    else:
...
@participantes.route('/wifi-visitante', methods=['POST', 'GET'])
@login_required
@email_verificado_required
def cadastro_wifi():
    ...
    if participante:
        ...
        return render_template('participantes/wifi_visitante.html', title='Cadastrar no WIFI-VISITANTE', form=form,
...
@participantes.route('/dados', methods=['POST', 'GET'])
@login_required
def dados():
    usuario = db.session.query(Usuario).filter_by(
        id=current_user.id).first()
    return render_template('participantes/dados.html', title='Dados Pessoais', usuario=usuario,
                           participante = db.session.query(Participante).filter_by(usuario=current_user).first())


@participantes.route('/dados-participante', methods=['POST', 'GET'])
@login_required
def dados_participante():
    ...
    ministrante = db.session.query(Ministrante).filter_by(
        usuario=current_user).first()
    return render_template('participantes/dados_participante.html', title='Dados de Participante', usuario=usuario,
...
@participantes.route('/verificacao/<token>')
...
def inscricao_minicursos():
    ...
    if agora >= evento_atual.abertura_minicursos_1_etapa and agora <= evento_atual.fechamento_minicursos_1_etapa\
        ...
        return render_template('participantes/inscricao_minicursos.html',
...
#@participantes.route('/inscricao-workshops')
@login_required
@email_verificado_required
def inscricao_workshops():
    ...
    return render_template('participantes/inscricao_workshops.html',
                           participante=db.session.query(Participante).filter_by(
                               usuario=current_user).first(), usuario=current_user, workshops=workshops)


@participantes.route('/inscricao-minicursos/<filtro>')
@login_required
@email_verificado_required
...
#@participantes.route('/inscricao-workshops/<filtro>')
@login_required
@email_verificado_required
...
@participantes.route('/inscrever-minicurso/<id>')
@login_required
@email_verificado_required
    ...
    if atv.vagas_disponiveis > 0:
        if agora >= evento_atual.abertura_minicursos_1_etapa and agora <= evento_atual.fechamento_minicursos_1_etapa:
            if participante.minicurso_etapa_1 is None:
                ...
                return render_template('participantes/inscricao_minicursos.html', participante=participante, usuario=current_user,
                                       minicursos=minicursos, acao="+", inscricao_liberada=1)
            else:
                minicursos = db.session.query(Atividade).filter_by(
                    tipo=tipo_atividade['minicurso'], id_evento=get_id_evento_atual()).filter(Atividade.titulo != None)
                return render_template('participantes/inscricao_minicursos.html', participante=participante,
                                       usuario=current_user,
...
#@participantes.route('/inscrever-workshop/<id>')
@login_required
@email_verificado_required
...
@participantes.route('/desinscrever-minicurso/<id>')
@login_required
@email_verificado_required
...
#@participantes.route('/desinscrever-workshop/<id>')
@login_required
@email_verificado_required
...
@participantes.route('/alterar-senha', methods=["POST", "GET"])
    ...
    if email_confirmado():
        ...
        if form.validate_on_submit():
            ...
            if pbkdf2_sha256.verify(form.senha_atual.data, usuario.senha):
                ...
                return render_template('participantes/alterar_senha.html', form=form, action=request.base_url, form_login=form_login,
...
@participantes.route('/esqueci-senha', methods=["POST", "GET"])
def esqueci_senha():
    ...
    if form.validate_on_submit():
        ...
        if usuario is not None:
            ...
            return render_template("participantes/esqueci_senha.html", status_envio_email=True, form=form, form_login=form_login)
...
@limiter.limit("1/minute")
@participantes.route('/confirmar-alteracao-senha/<token>', methods=["POST", "GET"])
def confirmar_alteracao_senha(token):
    ...
    if form.validate_on_submit():
        ...
        return redirect(url_for('views.login'))
    return render_template("participantes/alterar_senha.html", form=form, action=request.base_url, form_login=form_login, status_envio_email=True)


@participantes.route('/comprar-kit', methods=["POST", "GET"])
@login_required
@email_verificado_required
def comprar_kit():
    ...
    if current_user.email_verificado is True:
        ...
        if participante is not None:
            ...
            if form.validate_on_submit():
                ...
                return render_template('participantes/comprar_kit.html', usuario=current_user, participante = db.session.query(
...
@participantes.route('/executar-pagamento-kit', methods=["POST", "GET"])
@login_required
@email_verificado_required
        ...
        if pagamento.efetuado is False:
            if payer_id is not None:
                ...
                if payment.execute({"payer_id": payer_id}):
                    ...
                    return render_template('participantes/sucesso_pagamento_kit.html', usuario=current_user, participante=participante, form_login=form_login)
                else:
                    return render_template('participantes/erro_pagamento_kit.html', usuario=current_user, participante=participante, form_login=form_login)
                    print(payment.error)
        else:
            return render_template('participantes/pagamento_kit_efetuado.html', usuario=current_user, participante=participante, form_login=form_login)
    else:
        return redirect(url_for('.dashboard'))

@participantes.route('/cancelar-pagamento-kit', methods=["POST", "GET"])
@login_required
@email_verificado_required
def cancelar_pagamento_kit():
    return redirect(url_for('participantes.pagamentos'))

@participantes.route('/pagamentos', methods=["POST", "GET"])
@login_required
@email_verificado_required
def pagamentos():
    participante = db.session.query(Participante).filter_by(usuario=current_user, id_evento=get_id_evento_atual()).first()
    if participante is not None:
        ...
        if form.validate_on_submit():
            ...
            if pagamento is not None:
                ...
                db.session.add(pagamento)
                db.session.commit()
        return render_template('participantes/pagamentos.html', usuario=current_user, participante=participante, pagamentos=pagamentos, form=form)
    else:
        return redirect(url_for('.cadastro_participante'))

@participantes.route('/presencas', methods=["POST", "GET"])
@login_required
@email_verificado_required
def presencas():
    participante = db.session.query(Participante).filter_by(usuario=current_user, id_evento=get_id_evento_atual()).first()
    if participante is not None:
        ...
        return render_template('participantes/presencas.html', usuario=current_user, participante=participante, presencas=presencas, limites=limites)
    else:
        return redirect(url_for('.cadastro_participante'))

@participantes.route('/submeter-flag', methods=["GET", "POST"])
@login_required
def submeter_flag():
    form_login = LoginForm(request.form)
    ...
    if form.validate_on_submit():
        ...
        if(flag != None and flag not in participante.flags_encontradas and flag.ativa):
            ...
            db.session.flush()
            db.session.commit()
            return render_template("participantes/submeter_flag.html", usuario=current_user, form=form, form_login=form_login, participante=participante, status="aceita")
        elif flag in participante.flags_encontradas:
            return render_template("participantes/submeter_flag.html", usuario=current_user, form=form, form_login=form_login, participante=participante, status="já utilizada, safadinho")
        else:
            return render_template("participantes/submeter_flag.html", usuario=current_user, form=form, form_login=form_login, participante=participante, status="inválida")

    else:
        return render_template("participantes/submeter_flag.html", usuario=current_user, form=form, form_login=form_login, participante=participante, status=None)

@participantes.route('/submeter-feedback/<id_atividade>', methods=["GET", "POST"])
@login_required
@email_verificado_required
    ...
    if presenca is not None:
        if presenca.id_feedback is None and atividade_aconteceu(id_atividade):
            if form.validate_on_submit():
                ...
                return render_template('participantes/feedback.html', usuario=current_user, form_login=form_login, form=form, participante = db.session.query(
